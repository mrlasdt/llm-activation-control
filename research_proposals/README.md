...
| `BASELINES/` | A-LQR & PID-AcT reproductions + **MPC vs LQR vs PID** head-to-head | external control-law baselines on the cone plant | active — **head-to-head done (2026-06-13)**: on the `k=4` cone (R²=0.9994, Gemma-2-2b, AdvBench, StrongREJECT, matched effort) **P ≈ LQR ≈ MPC ≈ 0.72, tied within ~1 SE — predictive control gives NO de-refusal edge; the *bound* is the lever, not the law** (MPC only marginally more coherent; PID/feedforward can hurt). Theory-consistent (near-identity ⇒ lookahead inert; unconstrained ⇒ MPC=LQR, proved in code). Native impls unit-tested; paper-number reproductions **deferred (TODO)**. See `BASELINES/BASELINES_RESULTS.md` |
| `CALM/` | Coherence-Aware Lookahead MPC | **coherence inside the controller** — on-manifold density cost (P2) + token-domain KL feedback (P3) | active — **Phases 1–3 done (2026-06-13)**: tests the *strength↔coherence* trade-off the law-vs-law tie couldn't exploit. **THREE-NULL ARC.** P2: a coherence-aware `CoherenceMPC` (κ-weighted Mahalanobis density in CASA's QP) does NOT dominate the frontier (P≈MPC≈cMPC, spread ≤0.012) — the cone-space density doesn't predict genNLL (**r=0.20**). P3: a verified token-domain **KL-thermostat** (dual-cache Gemma-2 decode loop, matches `generate` 16/16) modulating strength on **ambient KL-to-unsteered** also ties static (best Δsr +0.018, no dominance) despite real modulation (α_std 18–63) — but **ambient KL IS a faithful coherence proxy, r=0.976**, validating the diagnosis. Token-MPC gated out (no signal). **Bottom line: across PTS→CALM, no control sophistication beats a fixed magnitude bound for cone de-refusal — actuator+bound is the lever, not the controller.** `calm_{mpc,token}.py` self-tested. See `CALM/CALM_RESULTS.md` |
