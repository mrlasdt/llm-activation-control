...
plane, so the only thing that varies is HOW the per-layer angle is chosen:
  ...
  2. Reset-1L (residual)     norm-preserving angle reset at the single selected layer's
                             RESIDUAL stream (model.layers.{L} output). NOTE: this is NOT
                             canonical Angular Steering — the published method hooks
  ...
  All four reset the RESIDUAL stream; only the angle-selection differs. Canonical Angular
  Steering (input_layernorm, single layer) is a separate, weaker actuator.
